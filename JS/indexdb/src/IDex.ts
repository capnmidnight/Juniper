import { hasValue, makeLookup, once, success, twonce } from "@juniper-lib/util";
...
export class IDex {
    ...
    get isOpen() { return hasValue(this.#db); }
    ...
    async assert(...storeDefs: StoreDef[]) {
        return await this.#modify("full", ...storeDefs);
    }

    async deleteStores(...storeNames: string[]) {
        if (this.isOpen) {
    ...
    async addStores(...storeDefs: StoreDef[]) {
        if (this.isOpen) {
    ...
    async changeStores(...storeDefs: StoreDef[]) {
    ...
    async #modify(mode: DiffMode, ...storeDefs: StoreDef[]) {
        if (this.isOpen) {
            throw new Error("Database is already open");
        }
        ...
        if (evt.type === "upgradeneeded") {
            if (mode === "remove") {
                ...
                if (upgradeNeeded) {
                    await usingAsync(maybeUpgrade.result, async (/** @type {IDBDatabase} */ db) =>
                        await this.#upgrade(db, maybeUpgrade.transaction, diff));
                }
            }
        }
        else {
            const version = maybeUpgrade.result.version;
            const upgradeNeeded = await usingAsync(maybeUpgrade.result, async (/** @type {IDBDatabase} */ db) =>
                await this.#inspect(db, diff, mode));
            if (upgradeNeeded) {
                ...
                await usingAsync(upgrade.result, async (/** @type {IDBDatabase} */ db) =>
                    await this.#upgrade(db, upgrade.transaction, diff));
            }
        }
    ...
    #assertIsUpgradeNeeded(evt: Event, upgrade: IDBOpenDBRequest) {
        if (evt.type === "success") {
    ...
    async #inspect(db: IDBDatabase, diff: DatabaseDiff, mode: DiffMode) {
        const { storesByName, storeNamesToAdd, storeNamesToRemove, storeNamesToChange,
            indexesByNameByStoreName, indexNamesToAddByStoreName, indexNamesToRemoveByStoreName } = diff;
        if (!hasValue(db)) {
            if (mode !== "remove") {
                // no database, add all the stores and indexes
                for (const [storeName, storeDef] of storesByName) {
                    storeNamesToAdd.add(storeName);
                    if (hasValue(storeDef.indexes)) {
            ...
            const storeNamesToScrutinize = new Set<string>();
    ...
    async #upgrade(db: IDBDatabase, transaction: IDBTransaction, diff: DatabaseDiff) {
...
function prepareDiff(storeDefs: StoreDef[]) {
    const indexesByNameByStoreName = new Map<string, Map<string, IDexDBIndexDef>>();
    const storeNamesToAdd = new Set<string>();
    const storeNamesToRemove = new Set<string>();
    const storeNamesToChange = new Set<string>();
    const indexNamesToAddByStoreName = new Map<string, Set<string>>();
    const indexNamesToRemoveByStoreName = new Map<string, Set<string>>();
...
async function usingAsync<T>(db: IDBDatabase, thunk: (db: IDBDatabase) => T) {
    try {
