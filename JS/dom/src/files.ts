import { hasValue, identity, isObject, once, pivot, singleton } from "@juniper-lib/util";
...
import { renderSVGElement } from "./util";
 ...
 * @param encoding
 */
export async function readAsText(blob: Blob | File | FileSystemFileHandle, encoding?: string) {
    if (isFileSystemFileHandle(blob)) {
        blob = await blob.getFile();
    }
