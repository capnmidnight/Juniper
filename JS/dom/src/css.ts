import { hasValue, isArray, isDefined, isNumber, isString, toString } from "@juniper-lib/util";
...
export function translate(x: 0 | string, y: 0 | string) {
...
export function hash(r: number | string, g: number | string, b: number | string, a?: number | string) {
    return "#" + [r, g, b, a].filter(hasValue).join("");
}

function cssList(name: string, ...args: (number | string)[]) {
    return `${name}(${args.filter(hasValue).join(", ")})`;
}
...
class SelectorPropSet extends PropSet {
    ...
    override get selector(): string {
        ...
        const selectorParts = super.selector
            .split(", ");
        const nestedSelector = selectorParts
            .map(v => superSelector + v)
            .join(", ");
