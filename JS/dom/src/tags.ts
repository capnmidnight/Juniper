import { arrayInsert, arrayRemove, Exception, hasValue, toString } from "@juniper-lib/util";
import { AbstractAppliable } from "./AbstractAppliable";
import { ClassList, DateTime, HRef, HtmlAttr, HtmlIDAttr, HtmlIsAttr, InnerHTML, Rel, TitleAttr, Type } from "./attrs";
 ...
 */
export function HtmlTag<T extends HTMLElement>(name: string, ...sub: ElementChild[]): T {
    sub = sub.flat(1);

    let tag: T;

    const finders: HtmlAttr[] = sub.filter((v: unknown): v is HtmlAttr =>
        v instanceof HtmlAttr
        && (v.name === "id" || v.name === "query" || v.name === "is"));

    let is: string = null;

    for (const finder of finders) {
        if (finder instanceof HtmlIDAttr) {
            tag = document.getElementById(finder.value as string) as T; // Cast to string since it has to be a string. 
            if (tag) {
...
export function StyleBlob(...content: PropSet[]): HTMLLinkElement {
    ...
    const rules = content
        .filter(p => p && (!p._subProps || p._subProps.length > 0))
        .map(toString);
 ...
 */
export function StyleTag(...rest: PropSet[]): HTMLElement {
 ...
 */
export function Time(date: Date, formattedDate: string): HTMLTimeElement {
    return HtmlTag(
        "time",
        DateTime(date),
