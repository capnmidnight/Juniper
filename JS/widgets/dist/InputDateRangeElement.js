import { first, formatDate, isDefined, last, singleton } from "@juniper-lib/util";
import { AutoComplete, ClassList, HtmlProp, InputDate, InputText, OnChange, OnClick, OnInput, SingletonStyleBlob, Value, border, boxShadow, fontSize, padding, perc, position, registerFactory, rem, rule, visibility } from "@juniper-lib/dom";
export function DateFormatter(callback) {
...
export class InputDateRangeElement extends HTMLElement {
    ...
    constructor() {
        ...
        SingletonStyleBlob("Juniper::Widgets::InputDateRange", () => [
            rule(".date-range", position("relative"), padding(0)),
            rule(".date-range:focus-within", boxShadow("0 0 0 0.25rem rgba(13, 110, 253, .25)")),
            rule(".date-range input[type=date]", position("absolute"), visibility("hidden")),
            rule(".date-range input[type=text]", position("relative"), fontSize(perc(80)), border("none"), padding(rem(.25), rem(.5))),
            rule(".date-range input[type=text]:focus", boxShadow("none"))
        ]);
        this.#inputs = [
            this.#front = InputText(AutoComplete("off"), Value("min - max"), OnClick((evt) => {
            ...
            this.#minDate = InputDate(OnInput(() => {
                this.#render();
                this.#selectMin();
            }), OnChange(() => {
                this.#maxDate.showPicker();
            ...
            this.#maxDate = InputDate(OnInput(() => {
                this.#render();
                this.#selectMax();
            }), OnChange(() => {
                this.dispatchEvent(inputEvt);
    ...
    connectedCallback() {
        if (!this.#minDate.isConnected) {
            this.append(this.#minDate, this.#maxDate, this.#front);
        }
...
export function InputDateRange(...rest) {
    return InputDateRangeElement.install()(...rest);
}
//# sourceMappingURL=InputDateRangeElement.js.map