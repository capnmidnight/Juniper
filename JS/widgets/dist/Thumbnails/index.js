...
export function registerThumbnails() {
    SingletonStyleBlob("Juniper::Widgets::Thumbnails", () => [
        rule(".thumbnail-view", position("fixed"), top(perc(50)), left(perc(50)), width(perc(75)), height(perc(80)), transform(translateFunc(perc(-50), perc(-50))), border("solid 1px grey"), backgroundColor("white"), padding(em(4)), margin("auto"), textAlign("center"), zIndex(150), rule(">img", height(perc(100))), rule(">iframe", width(perc(100)), height(perc(100)))),
        rule(".thumbnail", cursor("zoom-in")),
        rule(".img-thumbnail", width(em(2)))
    ]);
...
if (!thumbnailView) {
    thumbnailView = Div(ID(thumbnailViewerID), ClassList("thumbnail-view"), display("none"), Img(TitleAttr("Thumbnail")), IFrame(TitleAttr("Preview")));
}
