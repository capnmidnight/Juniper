import { ClassList, Div, HtmlAttr, I, Label, SingletonStyleBlob, TypedHTMLElement, backgroundColor, cursor, display, fontWeight, marginLeft, opacity, registerFactory, rule } from "@juniper-lib/dom";
...
export function Selectable(selectable) {
    return new HtmlAttr("selectable", selectable);
}
export class NavTreeItemElement extends TypedHTMLElement {
    ...
    constructor() {
        super();
        SingletonStyleBlob("Juniper::Widgets::NavTreeItem", () => rule("nav-tree-item", display("block"), marginLeft("1em"), rule("[selectable]>.controls", rule(">label", cursor("pointer")), rule(">i", cursor("default"))), rule(":has(>nav-tree-item)", rule("[open]", rule(">nav-tree-item", display("block")), rule(">.controls>i", cursor("zoom-out"))), rule(":not([open])", rule(">nav-tree-item", display("none")), rule(">.controls>i", cursor("zoom-in")))), rule("[selected]>.controls", fontWeight("bold"), backgroundColor("rgba(255 255 255 / 25%)")), rule(".highlighted>.controls", fontWeight("bold")), rule(":has(>nav-tree-item).dehighlighted", opacity(0.5)), rule(":not(:has(>nav-tree-item)).dehighlighted", display("none").important(true))));
        ...
        this.#controls = Div(ClassList("controls"), this.#collapser = I(pageWithCurl.value), this.#label = Label());
...
export function NavTreeItem(...rest) {
    return NavTreeItemElement.install()(...rest);
}
//# sourceMappingURL=NavTreeItemElement.js.map