...
import { Button, ClassList, Disabled, ElementChild, FAIcon, HtmlTag, Multiple, OnClick, Option, Query, QueryAll, Select, SlotAttr, StyleBlob, Template, TitleAttr, TypedHTMLElement, Value, display, gap, gridArea, gridTemplateAreas, gridTemplateColumns, gridTemplateRows, rule } from "@juniper-lib/dom";
...
export function InclusionList<T>(...rest: ElementChild[]): InclusionListElement<T> {
    return HtmlTag("inclusion-list", ...rest) as InclusionListElement<T>;
}
...
export class InclusionListElement<T> extends TypedHTMLElement<InclusionListEvents<T>> {
    ...
    constructor() {
        ...
        const template = singleton("Juniper::InclusionList::Template", () => Template(
            Select(
                Multiple(true),
        ...
        const style = singleton("Juniper::InclusionList::Style", () => StyleBlob(
            rule(":host",
                display("grid"),
        ...
        const move = (options: HTMLCollectionOf<HTMLOptionElement>, list: HTMLSelectElement) => {
            ...
            this.dispatchEvent(new TypedItemSelectedEvent(this.selectedItems[0], this.selectedItems));
        }
...
customElements.define("inclusion-list", InclusionListElement);