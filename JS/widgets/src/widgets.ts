import { formatPhoneNumber, hasMethod } from "@juniper-lib/util";
import {
    Button,
    ClassList,
    CustomData,
...
export function validateForm(form: HTMLFormElement): boolean {
    ...
    let valid = true;
    for (const input of form.querySelectorAll<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>("[required]")) {
        if (hasMethod(input, "checkValidity")
            && !input.checkValidity()) {
            if (valid) {
...
export function resetValidation(form: HTMLFormElement): void {
    ...
    for (const input of form.querySelectorAll(".invalid[required]")) {
