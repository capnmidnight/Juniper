import { first, formatDate, hasValue, last, singleton, stringCallback } from "@juniper-lib/util";
import { AutoComplete, ClassList, ElementChild, HtmlAttr, HtmlTag, InputDate, InputText, OnChange, OnClick, OnInput, StyleBlob, Value, border, boxShadow, fontSize, padding, perc, position, rem, rule, visibility } from "@juniper-lib/dom";

export function InputDateRange(...rest: ElementChild[]): InputDateRangeElement {
    return HtmlTag("input-date-range",
        ClassList("date-range"),
        ...rest
    );
}
...
export class InputDateRangeElement extends HTMLElement {
    ...
    constructor() {
        ...
        singleton("Juniper::InputDateRange::Style", () => {
    ...
    #render() {
        if (hasValue(this.maxValueAsDate)
            && hasValue(this.minValueAsDate)
...
customElements.define("input-date-range", InputDateRangeElement);