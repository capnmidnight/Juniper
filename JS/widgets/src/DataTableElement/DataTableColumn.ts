import {
    ...
    distinct, FieldName, formatDate, hasValue, identity, isNumber, isObject, isString,
...
import {
    ...
    Details, Disabled, Div, ElementChild, FAIcon, FormAttr, HRef, HtmlAttr, HtmlRender,
    InnerHTML, InputCheckbox, InputColor, InputNumber, InputText, LI, List, Max, Min, OnClick,
...
import { BaseColumnDef, ColumnType } from "../excel";
...
export class DataTableColumn<T> extends EventTarget {
    ...
    #styleGetter: (item: T) => HtmlAttr = always(null);
        ...
        if (this.#field) {
            ...
            if (this.#type === "date") {
                        ...
                        UL(
                            ...values.map(v => {
                                if (this.filterActive) {
                                    ...
                                    return LI(
                                        this.filterActive && StyleAttr({ "text-decoration": textDecoration }) || null,
                                        v
                                    );
                                }
                                else {
                ...
                this.#sortGetter = item => {
                    const value = this.#getter(item) as Array<ElementChild>;
                    return value.length;
                };
            }
