...
import {
    Button, ButtonReset, ClassList, ClassName, Clear, ColSpan,
    Div,
    ElementChild,
    FAIcon, FormAttr, FormTag, HtmlAttr, HtmlRender, HtmlTag, ID,
    OnClick,
    OnReset,
    OnScroll,
    QueryAll,
    StyleAttr, StyleBlob, TBody, TD, TFoot, TH,
    THead, TR, Table, TitleAttr,
    alignItems,
    backgroundColor,
    border,
    color,
    ...
    gridTemplateRows,
    height,
    hide, isModifierless, justifyContent, marginBottom, marginLeft, overflow,
    padding, px, rule,
    show,
    textAlign,
    textDecoration,
...
import { ColumnDef, DataTableColumn, DataTableItemEvent, isDataTableItemEvent } from "./DataTableColumn";
import { Paginator, PaginatorView } from "../Paginator";
import { BaseColumnDef, ColumnType, exportXLSX } from "../excel";

/**
 * An event that fires whenever the user does something that indicates a preference
 * for one of the rows in the DataTable.
 */
export class DataTableItemSelectedEvent<T> extends Event {
    #value;
...
export class DataTableElement<T> extends HTMLElement {
    #rootData: T[] = [];
    ...
    set selectedValue(v: T) {
        ...
        this.dispatchEvent(new DataTableItemSelectedEvent(v));
    }
    ...
    get paginated(): boolean { return !this.#paginator.disabled; }
    set paginated(v: boolean) { this.#paginator.disabled = !v; }
    ...
    constructor() {
        ...
        singleton("Juniper:Widgets:DataTable", () => {
        ...
        this.#resetFiltersButton = ButtonReset(
            StyleAttr({ display: "none" }),
            FAIcon("x"),
            FormAttr(this.#form.id),
    ...
    set pageSize(v: number) { this.setAttribute("pagesize", v.toFixed()); }

    connectedCallback() {
        this.#paginator.disabled = this.getAttribute("paginated") === "off";
        this.#paginator.autoSized = this.getAttribute("paginated") === "auto";
    ...
    #compareBy(column: DataTableColumn<T>) {
        this.#columns
            .filter(c => c !== column)
    ...
    filter() {
        ...
        if (filters.length === 0) {
            hide(this.#resetFiltersButton);
        }
        else {
            show(this.#resetFiltersButton);
        }
        ...
        if (this.#paginator.data.length === 0) {
            show(this.#noResultsBody, "table-row-group");
            hide(this.#resultsBody);
        }
        else {
            show(this.#resultsBody, "table-row-group");
            hide(this.#noResultsBody);
        }
    ...
    async exportXLSX(fileName: string, sheetName?: string, allData?: boolean) {
        exportXLSX(fileName, {
    ...
    #columnAlterer = {
        setVisible: (columnIndex: number, visible: boolean) => {
            this.#alterColumn(columnIndex, columnDef => {
                if (visible) {
                    columnDef.removeAttribute("hidden");
                }
                else {
                    columnDef.setAttribute("hidden", "");
                }
            });
        },
