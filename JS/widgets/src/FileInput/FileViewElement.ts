...
import { A, alignItems, Audio, backgroundColor, border, Button, ClassList, Controls, Disabled, display, Div, ElementChild, elementSetDisplay, FigCaption, flexDirection, flexGrow, gap, gridTemplateRows, height, I, Img, justifyContent, justifyItems, OnClick, OnInput, Option, overflow, padding, ReadOnly, registerFactory, rule, Select, SingletonStyleBlob, SpanTag, Src, Target, TextArea, textOverflow, TypedHTMLElement, Value, Video, whiteSpace, width } from "@juniper-lib/dom";
...
export class FileViewElement extends TypedHTMLElement<FileViewElementEvents> {
    ...
    readonly #deleter: HTMLButtonElement;
    readonly #caption: HTMLElement;
    readonly #type: HTMLElement;
    readonly #size: HTMLElement;
    readonly #preview: HTMLElement;
    readonly #controls: HTMLElement;
    readonly #indexSelector: HTMLSelectElement;
    ...
    constructor() {
        ...
        SingletonStyleBlob("Juniper::Cedrus::FileView", () =>
            rule("file-view",
                display("grid"),
        ...
        this.#titleBar = Div(
            ...
            this.#deleter = Button(
                ...
                I(wastebasket.value),
                OnClick(() => this.dispatchEvent(new RemovingEvent(this.file)))
            )
        );
        ...
        this.#caption = FigCaption(
            ClassList("controls"),
            this.#type = SpanTag("No file selected"),
            this.#size = SpanTag(" 0.0 KB")
        );
        ...
        this.#controls = Div(
            ...
            this.#indexSelector = Select(
                OnInput(() => this.dispatchEvent(new IndexChangedEvent(this.#indexSelector.selectedIndex))
                )
            ),

            this.#moveRight = Button(
                I(rightwardsArrow.value),
                OnClick(() => {
    ...
    setPosition(index: number, count: number) {
        this.#indexSelector.replaceChildren(
            ...generate(0, count).map(i => Option(i + 1)
            )
        );
    ...
    #lastInfo: IBasicFile & { path: string; } = null;

    async #render() {
        let title = "Unknown file...";
        let type = "unknown/unknown";
        let formattedSize = "N/A";
        let preview: HTMLElement = Div("Preview unavailable...");;
    ...
    async #getFileInfo(): Promise<IBasicFile & { path: string }> {
...
export function FileView(...rest: ElementChild<FileViewElement>[]) {
