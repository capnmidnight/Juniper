...
import { alignItems, Disabled, display, Div, ElementChild, elementSetDisplay, flexDirection, flexGrow, gap, gridAutoRows, gridTemplateColumns, HtmlRender, InputFile, OnDragEnd, OnDragOver, OnDrop, OnInput, ReadOnly, registerFactory, repeat, rule, SingletonStyleBlob, TypedHTMLElement } from "@juniper-lib/dom";
import { OnRemoving } from "../ArrayViewElement";
import { FileAttr } from "./FileAttr";
import { FileView, FileViewElement, FileViewValue } from "./FileViewElement";
...
export class FileInputElement extends TypedHTMLElement {
    ...
    readonly #tempFiles = new Array<FileViewElement>();

    constructor() {
        super();

        SingletonStyleBlob("Juniper::Cedrus::FileInput", () =>
            rule("file-input",
                display("flex"),
    ...
    get fileViews() { return this.#views.querySelectorAll<FileViewElement>("file-view"); }
    ...
    get placeholder(): string { return null; }
    ...
    addFiles(files: FileViewValue[]) {
        this.#addFiles(files, false);
    }

    #addFiles(files: FileViewValue[], temp: boolean) {
...
export function FileInput(...rest: ElementChild<FileInputElement>[]) {
