import { IResponse, isObject, isString } from "@juniper-lib/util";
 ...
 * @returns
 */
function marshalDates<T>(response: IResponse<T>): IResponse<T> {
...
export class FetchingService implements IFetchingService {
    ...
    sendNothingGetObject<T>(request: IRequest, progress: IProgress): Promise<IResponse<T>> {
        return this.#impl.sendNothingGetSomething<"json", T>("json", request, progress).then(marshalDates);
    }

    sendObjectGetObject<T>(request: IRequestWithBody, progress: IProgress): Promise<IResponse<T>> {
        return this.#impl.sendSomethingGetSomething<"json", T>("json", request, this.defaultPostHeaders, progress).then(marshalDates);
    }
