...
import { isFlatEntityModel, isFlatEntityTypeModel } from '../models';
export class CedrusOrgChart extends TypedEventTarget {
    #container;
    #chart;
    #cardStyles;
    ...
    constructor(container) {
        ...
        SingletonStyleBlob("Juniper::Cedrus::CedrusOrgChart", () => [
            rule(":root", cssVarDecl("sat", perc(90)), cssVarDecl("lgh1", perc(95)), cssVarDecl("lgh2", perc(70)), cssVarDecl("lgh3", perc(40)), cssVarDecl("ba", "0.5"), cssVarDecl("bx", px(5)), cssVarDecl("by", px(5)), cssVarDecl("bb", px(10))),
            rule("g.nodes-wrapper > g", cursor("default")),
            rule(".link", stroke("black"), strokeWidth(px(5))),
            rule(".cardName", fontSize(px(15)), display("flex"), gap(px(5)), margin(px(5)), justifyContent("space-between")),
            rule(".cardBody", backgroundColor("white"), borderRadius(px(10)), border("1px solid grey"), boxShadow("hsla(0, 0%, 0%, var(--ba)) var(--bx) var(--by) var(--bb)"), padding(em(1)))
        ]);
    ...
    #makeNode(node) {
        ...
        return Div(ClassList("cardBody", entityTypeName.replaceAll(" ", "-").replaceAll(/-{2,}/g, "-")), CustomData("type-name", entityTypeName), Div(ClassList("cardType"), entityTypeName ?? "Unknown"), Div(ClassList("cardName"), entity.name, ...Optional(isEntity, Button(CustomData("entity-id", entity.id), openFileFolder.value))), ...Optional(titleText, Div(ClassList("cardTitle"), titleText))).outerHTML;
    }
    #clickNode(node) {
        ...
        const expandIds = new Set(clones.map(clone => clone.id));
    ...
    #createFakeEntity(type, name, children) {
        const fakeEntity = {
    ...
    setTree(tree, groupByEntityType = false) {
        this.#tree = tree;
        this.#data = [];
        ...
        const queue = roots.map(root => [root, null]);
        ...
        const rules = distinct(this.#getEntityTypes().map(type => type.name))
            ...
            return rule(`.cardBody[data-type-name='${type}']`, backgroundColor(`hsl(${angle}, var(--sat), var(--lgh1))`), boxShadow(`hsla(${angle}, var(--sat), var(--lgh2), var(--ba)) var(--bx) var(--by) var(--bb)`), border(`1px solid hsl(${angle}, var(--sat), var(--lgh3))`));
        });
...
//# sourceMappingURL=CedrusOrgChart.js.map