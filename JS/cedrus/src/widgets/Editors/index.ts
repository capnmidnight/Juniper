...
const SingleBooleanEditor = makeSingleInputElement(
    "Boolean",
    InputCheckbox,
    input => input.value.toString(),
    identity,
    input => input.checked,
    (input, v) => input.checked = v
);

const SingleIntegerEditor = makeSingleInputElement(
    "Integer",
    () => InputNumber(Step(1), Pattern("-?\\d+")),
    input => input.value.toString(),
    identity,
...
const SingleDecimalEditor = makeSingleInputElement(
    "Decimal",
...
const SingleCurrencyEditor = makeSingleInputElement(
    "Currency",
    () => InputCurrency(Step(0.01)),
    input => input.value.toString(),
    identity,
    input => input.valueAsNumber,
    (input, value) => input.valueAsNumber = value
...
const SingleStringEditor = makeSingleInputElement(
    "String",
    InputText,
    input => input.value,
    identity,
...
const SingleDateEditor = makeSingleInputElement(
    "Date",
    InputDate,
    input => input.value.toString(),
    identity,
    input => input.valueAsDate,
    (input, value) => input.valueAsDate = value
);

const SingleLinkEditor = makeSingleInputElement(
    "Link",
    InputText,
    input => A(
        HRef(input.value),
...
const SingleLongTextEditor = makeSingleInputElement(
    "LongText",
    TextArea,
    input => input.value,
    identity,
...
const SingleEnumerationEditor = makeSingleInputElement(
    "Enumeration",
    TypedSelect<string>,
    ...
    input => input.value,
    (input, value) => input.value = value,
    (input, values) => input.data = values
);

export function isFileEditor(editor: IPropertyEditorElement): editor is (ArrayFileEditorElement | SingleFileEditorElement) {
