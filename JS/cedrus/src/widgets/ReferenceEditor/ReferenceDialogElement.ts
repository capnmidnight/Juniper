...
import { ClassList, display, ElementChild, For, gap, H1, ID, InputDate, InputText, Label, OnInput, PlaceHolder, registerFactory, Required, rule, SingletonStyleBlob, TitleAttr, TypeAttr } from "@juniper-lib/dom";
import { BaseDialogElement, Cancelable, DataAttr, DataColumn, DataTable, DataTableElement, FieldAttr, FileInput, FileInputElement, PropertyGroup, PropertyList, PropertyListElement, TabPane, TabPanel, TabPanelElement, TypedSelect, TypedSelectElement } from "@juniper-lib/widgets";
import { CedrusDataAPI } from "../../adapters";
import { ENTITY_TYPE, EntityModel, PROPERTY_TYPE } from "../../models";
import { ReferenceDataManager, ReferenceModel, referencePropertyTypes, ReferencePropertyTypeTypes } from "./ReferenceDataManager";
...
const protocols: ProtocolTypes[] = [
    "file://",
    "http://",
    "https://"
];

export class ReferenceDialogElement extends BaseDialogElement<EntityModel, EntityModel> {

    readonly #tabs: TabPanelElement;
    readonly #properties: PropertyListElement;
    readonly #references: DataTableElement<ReferenceModel>;
    readonly #nameInput: HTMLInputElement;
    readonly #authorsInput: HTMLInputElement;
    readonly #publicationDateInput: HTMLInputElement;
    readonly #typeSelector: TypedSelectElement<ReferencePropertyTypeTypes>;
    readonly #protocolInput: TypedSelectElement<ProtocolTypes>;
    readonly #pathInput: HTMLInputElement;
    readonly #fileInput: FileInputElement;
    readonly #dataManager: ReferenceDataManager;

    constructor() {

        SingletonStyleBlob("Juniper::Cedrus::ReferenceDialogElement", () => [
            rule(".reference-editor-dialog",
        ...
        const tabs = TabPanel(
            TabPane("Select existing",
                DataTable(
                    ID(id + "References"),
                    ClassList("table", "table-striped", "table-borderless", "table-hover"),
                    DataColumn(TypeAttr("text"), FieldAttr("name"), TitleAttr("Name")),
                    DataColumn(TypeAttr("text"), FieldAttr("authors"), TitleAttr("Authors")),
                    DataColumn(TypeAttr("date"), FieldAttr("date"), TitleAttr("Published")),
            ...
            TabPane("Upload new",
                PropertyList(
                    ...
                    InputText(
                        ...
                        TitleAttr("Reference Document Title")
                    ),

                    Label(For(id + "Authors"), "Authors"),
                    InputText(
                        ID(id + "Authors"),
                        Required(true),
                        PlaceHolder("Enter authors..."),
                        TitleAttr("Reference Document Authors")
                    ),

                    Label(For(id + "Date"), "Publication Date"),
                    InputDate(
                        ID(id + "Date"),
                        PlaceHolder("Select publication date..."),
                        TitleAttr("Reference Publication Date")
                    ),
                    ...
                    TypedSelect(
                        ...
                        TitleAttr("Reference Type"),
                        DataAttr(referencePropertyTypes),
                        OnInput(() => {
                    ...
                    PropertyGroup("ReferenceFile",
                        Label(For(id + "File"), "File"),
                    ...
                    PropertyGroup("ReferenceLink",
                        Label(For(id + "Protocol"), "Protocol"),
                        TypedSelect(
                            ID(id + "Protocol"),
                            DataAttr(protocols)
                        ),

                        Label(For(id + "Link"), "Path"),
                        InputText(
                            ID(id + "Link"),
                            OnInput(() => {
...
export function ReferenceDialog(...rest: ElementChild<ReferenceDialogElement>[]) {
