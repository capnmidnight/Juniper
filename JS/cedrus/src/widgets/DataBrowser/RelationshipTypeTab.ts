...
import { RelationshipTypeModel } from "../../models";
import { DataBrowserPane, OnDisabled, OnShown } from "./DataBrowserPane";

export function RelationshipTypeTab() {

    let newRelationshipTypeParentRole: HTMLInputElement;
    let newRelationshipTypeChildRole: HTMLInputElement;


    const dt = DataTable<RelationshipTypeModel>(
        ...
        DataColumn(TypeAttr("text"), FieldAttr("parentRole"), TitleAttr("Parent Role")),
        DataColumn(TypeAttr("text"), FieldAttr("childRole"), TitleAttr("Child Role")),
        DataColumn(TypeAttr("button"), IconAttr("trash"), EventAttr("delete"), HeaderAttr(""), TitleAttr("Delete"))
    );

    const pane = DataBrowserPane("Relationship Types",
        ...
        FieldSet(
            Legend("Create new Relationship Type"),
            PropertyList(
                Label(For("newRelationshipTypeParentRole"), "Parent Role"),
                newRelationshipTypeParentRole = InputText(
                    ID("newRelationshipTypeParentRole"),
                    Required(true),
                    PlaceHolder("Enter parent role..."),
                    TitleAttr("Relationship Parent Role Name")
                ),

                Label(For("newRelationshipTypeChildRole"), "Child Role"),
                newRelationshipTypeChildRole = InputText(
                    ID("newRelationshipTypeChildRole"),
                    PlaceHolder("Enter child role..."),
                    TitleAttr("Relationship Child Role Name")
                ),
            ),
            Button("Save", OnClick(() => withErrorAlert(async () => {
                const ds = await CedrusDataAPI.dataSourceTask;
                const relationshipType = await ds.setRelationshipType(
                    newRelationshipTypeParentRole.value,
                    newRelationshipTypeChildRole.value
                    || newRelationshipTypeParentRole.value
                )
