...
import { ArrayEditor, ArrayEditorElement, DataColumn, DataTable, EventAttr, FieldAttr, HeaderAttr, LabelField, OnItemSelected, PropertyList, SortKeyField, TypedSelect, TypedSelectElement, ValueField } from "@juniper-lib/widgets";
...
export function UserTab() {
    ...
    let rolesSelection: ArrayEditorElement<TypedSelectElement<RoleModel>>;
    let saveButton: HTMLButtonElement;

    const dt = DataTable<UserModel>(
        ...
        DataColumn(TypeAttr("text"), FieldAttr("email"), TitleAttr("Email")),
        DataColumn(TypeAttr("csv"), FieldAttr("roles"), TitleAttr("Roles")),
        DataColumn(TypeAttr("button"), IconAttr("trash"), EventAttr("delete"), HeaderAttr(""), TitleAttr("Delete")),
        OnItemSelected<UserModel>(evt => withErrorAlert(async () => {
    ...
    const props = PropertyList(
        ...
        emailInput = InputEmail(
            ...
            TitleAttr("User Email Address"),
            Required(true)
        ),
        
        Label(For("newUserRoles"), "Roles"),
        rolesSelection = ArrayEditor(
            TypedSelect<RoleModel>(
                ID("newUserRoles"),
                SlotAttr("selector")
            ),
            LabelField<RoleModel>(v => v.name),
            ValueField<RoleModel>(v => v.name),
            SortKeyField<RoleModel>(v => v.name)
        )
    );

    const pane = DataBrowserPane("Users",
