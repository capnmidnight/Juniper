...
import { DataColumn, DataTable, EventAttr, FieldAttr, HeaderAttr, LabelField, Nullable, OnItemSelected, PropertyList, SortKeyField, TypedSelect, TypedSelectElement, ValueField } from "@juniper-lib/widgets";
...
export function EntityTypeTab() {
    ...
    let parentEntityType: TypedSelectElement<EntityTypeModel>;
    let isPrimary: HTMLInputElement;
    let newEntityTypeName: HTMLInputElement;


    const dt = DataTable<EntityTypeModel>(
        ...
        DataColumn(TypeAttr("boolean"), FieldAttr("isPrimary"), TitleAttr("Is Primary")),
        DataColumn(TypeAttr("text"), FieldAttr("parent.name"), TitleAttr("Parent")),
        DataColumn(TypeAttr("button"), IconAttr("trash"), EventAttr("delete"), HeaderAttr(""), TitleAttr("Delete")),
        OnItemSelected<EntityTypeModel>(evt => {
    ...
    const pane = DataBrowserPane("Entity Types",
        ...
        FieldSet(
            Legend("Create new Entity Type"),
            PropertyList(
                Label(For("newEntityTypeName"), "Name"),
                newEntityTypeName = InputText(
                    ID("newEntityTypeName"),
                    Required(true),
                    PlaceHolder("Enter name..."),
                    TitleAttr("Entity Type Name")
                ),

                Label(For("newEntityTypeIsPrimary"), "Is Primary"),
                isPrimary = InputCheckbox(
                    ID("newEntityTypeIsPrimary"),
                    TitleAttr("Show Entity Type in Main Menu")
                ),

                Label(For("newEntityTypeParent"), "Parent Entity Type"),
                parentEntityType = TypedSelect(
                    ID("newEntityTypeParent"),
                    PlaceHolder("Select parent type..."),
                    TitleAttr("Entity Type Parent"),
                    Nullable(true),
