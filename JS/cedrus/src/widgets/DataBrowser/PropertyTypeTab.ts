...
import { DataAttr, DataColumn, DataTable, EventAttr, FieldAttr, HeaderAttr, OnItemSelected, PropertyList, SortKeyField, TypedInput, TypedInputElement, TypedSelect, TypedSelectElement } from "@juniper-lib/widgets";
import { CedrusDataAPI } from "../../adapters";
import { DataType, DataTypeValues, PropertyTypeModel, StorageType, storageTypesByDataType } from "../../models";
...
export function PropertyTypeTab() {
    ...
    let dataTypeInput: TypedSelectElement<DataType>;
    let storageTypeInput: TypedSelectElement<StorageType>;
    let unitsCategory: TypedInputElement<string>;


    const dt = DataTable<PropertyTypeModel>(
        ...
        DataColumn(TypeAttr("text"), FieldAttr("type"), TitleAttr("Type")),
        DataColumn(TypeAttr("text"), FieldAttr("storage"), TitleAttr("Storage")),
        DataColumn(TypeAttr("text"), FieldAttr("unitsCategory"), TitleAttr("Units")),
        DataColumn(TypeAttr("text"), FieldAttr("description"), TitleAttr("Description")),
    ...
    const pane = DataBrowserPane("Property Types",
        ...
        dt,
        FieldSet(
            Legend("Create new Property Type"),
            PropertyList(
                Label(For("newPropertyTypeName"), "Name"),
                name = InputText(
                ...
                Label(For("newPropertyTypeDescription"), "Description"),
                description = TextArea(
                    ID("newPropertyTypeDescription"),
                    Required(true),
                    PlaceHolder("Enter description..."),
                    TitleAttr("Property Type Description")
                ),

                Label(For("newPropertyTypeDataType"), "Data Type"),
                dataTypeInput = TypedSelect<DataType>(
                    ID("newPropertyTypeDataType"),
                    DataAttr(DataTypeValues.filter(v => storageTypesByDataType.has(v) && storageTypesByDataType.get(v).length > 0)),
                    Required(true),
                    PlaceHolder("Select data type..."),
                    TitleAttr("Property Type Data Type"),
                    OnInput(() => setStorageTypes(dataTypeInput.selectedItem, storageTypeInput.selectedItem))
                ),

                Label(For("newPropertyTypeStorageType"), "Storage Type"),
                storageTypeInput = TypedSelect<StorageType>(
                    ID("newPropertyTypeStorageType"),
                    Required(true),
                    PlaceHolder("Select storage type..."),
                    TitleAttr("Property Type Storage Type")
                ),

                Label(For("newPropertyTypeUnitsCategory"), "Units Category"),
                unitsCategory = TypedInput<string>(
                    ID("newPropertyTypeUnitsCategory"),
                    SortKeyField<string>(identity),
                    Required(true),
                    PlaceHolder("Select units category..."),
                    TitleAttr("Property Type Units Category")
                )
            ),
    ...
    function setStorageTypes(dataType: DataType, storageType: StorageType) {
