...
import { ArrayEditor, ArrayEditorElement, DataColumn, DataTable, EventAttr, FieldAttr, HeaderAttr, LabelField, OnItemSelected, PropertyList, SortKeyField, TypedInput, TypedInputElement, ValueField } from "@juniper-lib/widgets";
...
export function RoleTab() {
    ...
    let roleName: HTMLInputElement;
    let users: ArrayEditorElement<TypedInputElement<UserModel>>;


    const dt = DataTable<RoleModel>(
        ...
        OnItemSelected<RoleModel>(evt => withErrorAlert(async () => {
    ...
    const pane = DataBrowserPane("Roles",
        ...
        dt,
        FieldSet(
            Legend("Create new Role"),
            PropertyList(
                Label(For("newRoleName"), "Name"),
                roleName = InputText(
                    ID("newRoleName"),
                    Required(true),
                ...
                Label(For("newRoleUsers"), "Users"),
                users = ArrayEditor(
                    TypedInput<UserModel>(
                        ID("newRoleUsers"),
                        SlotAttr("selector"),
                        PlaceHolder("Select user..."),
                        TitleAttr("User in Role")
                    ),
                    LabelField<UserModel>(v => v.name),
                    ValueField<UserModel>(v => v.name),
                    SortKeyField<UserModel>(v => v.name)
                )
            ),
