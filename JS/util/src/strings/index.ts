...
export function formatPhoneNumber(countryCode: string, areaCode: string, exchange: string, extension: string) {
    if (!hasValue(exchange) || !hasValue(extension)) {
        return null;
    }
...
const DEFAULT_ALPHABET = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_0123456789-";

export function stringRandom(length: number, alphabet = DEFAULT_ALPHABET) {
    let output = "";
    for (let i = 0; i < length; ++i) {
        // As these strings might be used as identifiers,
        // don't allow digits to start the string.
        const max = i === 0
            ? alphabet.indexOf("0")
            : alphabet.length;
