...
import { ActivityDetector, canChangeAudioOutput } from "@juniper-lib/audio";
import { Button, Checked, ClassList, Clear, Div, For, H1, HtmlRender, ID, InputCheckbox, Label, Max, Meter, Min, OnClick, OnInput, Option, Select, Selected, SlotAttr, Step, TitleAttr, Value, display, elementSetDisplay, em, margin, paddingRight } from "@juniper-lib/dom";
import { AssetFile } from "@juniper-lib/fetcher";
import { Audio_Mpeg } from "@juniper-lib/mediatypes";
import { InputRangeWithNumber, PropertyGroup, PropertyList } from "@juniper-lib/widgets";
...
function makeDeviceSelector(selector, devices, curDevice) {
    HtmlRender(selector, Clear(), Option(Value(""), "NONE"), ...devices.map((device) => Option(Selected(curDevice
        && device.deviceId === curDevice.deviceId), Value(device.deviceId), device.label)));
}
export class DeviceDialog extends BaseDialogElement {
    constructor(fetcher, devices, audio, microphones, webcams, DEBUG = false) {
        const id = stringRandom(12);
        const webcamSelector = Select(ID(id + "Webcams"), OnInput(async () => {
            const deviceId = this.webcamSelector.value;
            const device = this.camLookup.get(deviceId);
            await this.webcams.setDevice(device);
        }));
        const microphoneSelector = Select(ID(id + "Microphones"), display("inline-block"), OnInput(async () => {
        ...
        const micVolumeControl = InputRangeWithNumber(ID(id + "MicVolume"), Min(0), Max(100), Step(1), Value(0), OnInput(() => {
            this.microphones.gain.setValueAtTime(this.micVolumeControl.valueAsNumber / 100, 0);
        }));
        const micLevels = Meter(ID(id + "Levels"), display("inline-block"));
        const properties = PropertyList(SlotAttr("modal-body"), PropertyGroup(CAM_GROUP, Label(For(id + "Webcams"), "Webcams"), webcamSelector), PropertyGroup(MIC_GROUP, Label(For(id + "Microphones"), "Microphones"), microphoneSelector, Label(For(id + "MicVolume"), "Volume"), micVolumeControl, Label(For(id + "Levels"), "Levels"), micLevels));
        super(H1("Devices"), properties);
        ...
        HtmlRender(this.body, paddingRight("2em"), this.properties);
        if (canChangeAudioOutput) {
            this.properties.append(Label(For(id + "Speackers"), "Speakers"), this.speakers = Select(ID(id + "Speakers"), OnInput(async () => {
        ...
        this.properties.append(Label(For(id + "SpkrVolume"), "Volume"), this.spkrVolumeControl = InputRangeWithNumber(ID(id + "SpkrVolume"), Min(0), Max(100), Step(1), Value(0), OnInput(() => this.audio.destination.volume
            = this.spkrVolumeControl.valueAsNumber / 100)), this.testSpkrButton = Button("Test", TitleAttr("Test audio"), margin(em(0.5)), OnClick(async () => {
            this.testSpkrButton.disabled = true;
            await clipLoaded;
            await this.audio.playClipThrough("test-audio");
            this.testSpkrButton.disabled = false;
        })), Label(For(id + "Headphones"), "Using headphones"), this.useHeadphones = InputCheckbox(ID(id + "Headphones"), Checked(this.audio.useHeadphones), OnInput(() => {
        ...
        this.dialog.addEventListener("showing", async (evt) => {
            ...
            if (canChangeAudioOutput) {
                ...
                HtmlRender(this.speakers, Clear(), ...spkrs.map((device) => Option(Value(device.deviceId), device.label)));
