...
import { IFetcher, createFetcher, unwrapResponse } from "@juniper-lib/fetcher";
 ...
 * window to close.
 */
export class AppShellClosingEvent extends TypedEvent<typeof CLOSING> {
    constructor() {
        super(CLOSING);
    }
}
 ...
 * @see https://github.com/capnmidnight/Juniper/tree/J2/NETCore/AppShell
...
export class AppShell extends TypedEventTarget<AppShellEventMap> {    
    ...
    /**
     * Triggered when {@link @juniper-lib/appshell!AppShell.close} is called,
     * to allow listeners the chance to set `preventDefault`.
     * @event
     */
    ...
    async #setButton(button: HTMLButtonElement, enabled: Promise<boolean>, onClick: eventHandler<Event>) {
        if (button) {
            button.addEventListener("click", onClick);
            button.disabled = !(await enabled);
        }
    }
}