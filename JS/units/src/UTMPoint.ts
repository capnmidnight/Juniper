import { ICloneable, deg2rad, isDefined, isObject } from "@juniper-lib/util";
...
export class UTMPoint implements IUTMPoint, ICloneable {
    ...
    static centroid(points: UTMPoint[]): UTMPoint {
        ...
        const vec = points
            .map((p) => p
                ...
                .toVec3())
            .reduce((a, b) =>
                a.scaleAndAdd(b, scale), new Vec3());
    ...
    constructor(eastingOrCopy?: number | IUTMPoint, northing?: number, altitude?: number, zone?: number) {
        if (isObject(eastingOrCopy)) {
            this.#easting = eastingOrCopy.easting;
            this.#northing = eastingOrCopy.northing;
            this.#altitude = eastingOrCopy.altitude;
            this.#zone = eastingOrCopy.zone;
        }
        else {
            this.#easting = eastingOrCopy || 0;
            this.#northing = northing || 0;
    ...
    static #A(cosPhi: number, lng: number, utmz: number) {
        const zcm = 3 + (6 * (utmz - 1)) - 180;
        return deg2rad(lng - zcm) * cosPhi;
    }

    static #getZoneWidthAtLatitude(lat: number) {
