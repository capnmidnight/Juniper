import { ICloneable, isNullOrUndefined, isObject, rad2deg } from "@juniper-lib/util";
...
export class LatLngPoint implements ILatLngPoint, ICloneable {
    ...
    static #parseDMS(value: string): number {
    ...
    static #parseDMSPair(value: string): LatLngPoint | null {
        const parts = value.split(",");
        if (parts.length == 2) {
    ...
    static #parseDecimal(value: string): LatLngPoint | null {
        const parts = value.split(",");
        if (parts.length == 2) {
     ...
     */
    static #toDMS(value: number, negative: string, positive: string, sigfigs: number): string {
