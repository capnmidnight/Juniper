export function distance_WagnerFischer_Damerau_Levenshtein(a, b) {
    if (!a) {
...
export function similarity_WagnerFischerDamerauLevenshtein(a, b) {
    if (!a) {
        throw new Error(`Argument ${a} is null`);
    }
    if (!b) {
...
/// <returns></returns>
export function similarity_JaroWinkler(a, b) {
    if (!a) {
        throw new Error(`Argument ${a} is null`);
    }
    if (!b) {
    ...
    const matchesA = new Array(a.length);
    const matchesB = new Array(b.length);
    let numCommon = 0;
