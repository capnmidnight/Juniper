...
namespace Juniper.Input.Pointers
{
    public interface IPointerDevice
    {
        ...
        bool IsDisabled { get; }
        bool IsEnabled { get; }
        bool LockedOnTarget { get; set; }

        Type ButtonType { get; }

        PhysicsRaycaster Raycaster { get; }
        Camera EventCamera { get; }
        IEventSystemHandler EventTarget { get; set; }
        UnifiedInputModule InputModule { get; set; }

        Vector3 InteractionDirection { get; }
        Vector3 InteractionEndPoint { get; }
        bool IsDragging { get; }
        float MaximumPointerDistance { get; }
        float MinimumPointerDistance { get; }
        int PointerID { get; set; }
        string ProbeName { get; }

        Vector2 ScrollDelta { get; }
        Vector3 WorldPoint { get; }
        Vector2 ViewportPoint { get; }
