...
import { display, margin, styles } from "./css";
...
export function BackgroundAudio(autoplay, mute, looping, ...rest) {
    return Audio(playsInline(true), controls(false), muted(mute), autoPlay(autoplay), loop(looping), styles(display("none")), ...rest);
}
export function BackgroundVideo(autoplay, mute, looping, ...rest) {
