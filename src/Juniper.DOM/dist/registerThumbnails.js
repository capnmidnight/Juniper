...
Style(rule(".thumbnail-view", position("fixed"), top("50%"), left("50%"), width("75%"), height("80%"), transform("translate(-50%, -50%)"), border("solid 1px grey"), backgroundColor("white"), padding("4em"), margin("auto"), textAlign("center"), zIndex(15)), rule(".thumbnail", cursor("zoom-in")), rule(".thumbnail-view > img", height("100%")), rule(".thumbnail-view > iframe", width("100%"), height("100%")), rule(".img-thumbnail", width("2em")));
...
if (!thumbnailView) {
    thumbnailView = Div(id(thumbnailViewerID), className("thumbnail-view"), styles(display("none")), Img(title("Thumbnail")), IFrame(title("Preview")));
}
