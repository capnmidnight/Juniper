...
export function onProgress(callback, opts) { return new HtmlEvt("progress", (evt) => callback(evt), opts); }
...
export function applyFakeProgress(file, elem, prog) {
    if (prog) {
        elementApply(elem, onProgressCallback(prog), onLoadStart(() => prog.report(0, 1, `${file} loading`)), onLoad(() => prog.report(1, 1, `${file} loaded`)));
    }
}
