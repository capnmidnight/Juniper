import { Audio, autoPlay, BackgroundAudio, display, elementApply, id, playsInline, srcObject, styles } from "juniper-dom";
import { arrayRemove, arraySortedInsert, isDefined, isMobileVR, isOculusBrowser, oculusBrowserVersion, stringToName, TypedEvent, TypedEventBase } from "juniper-tslib";
import { audioReady, BiquadFilter, BufferSource, disconnect, DynamicsCompressor, Gain, MediaElementSource, MediaStreamDestination, MediaStreamSource } from "./nodes";
...
export class AudioManager extends TypedEventBase {
    ...
    createBasicClip(id, path, vol, onProgress) {
     ...
     */
    async createClip(id, path, looping, autoPlaying, spatialize, randomize, vol, effectNames, onProgress) {
    ...
    getSourceTask(id, path, looping, autoPlaying, onProgress) {
    ...
    async createSourceFromFile(id, path, looping, autoPlaying, onProgress) {
        if (useElementSourceForClips) {
            const { content: elem, fileName } = await this.fetcher
                .get(path)
                .progress(onProgress)
                .audio(autoPlaying, looping);
            return MediaElementSource(stringToName("audio-element-source", fileName, id, path), this.audioCtx, elem);
        }
        else {
            ...
            return BufferSource(stringToName("audio-buffer-source", fileName, id, path), this.audioCtx, {
                buffer: data,
                loop: looping
            });
        }
    }
    createSourceFromStream(id, stream) {
        const elem = BackgroundAudio(true, !useElementSourceForUsers, false, srcObject(stream));
        if (useElementSourceForUsers) {
            return MediaElementSource(stringToName("media-element-source", id, stream.id), this.audioCtx, elem);
        }
        else {
            return MediaStreamSource(stringToName("media-stream-source", id, stream.id), this.audioCtx, stream);
        }
    }
    makeClip(source, id, path, spatialize, randomize, autoPlaying, vol, ...effectNames) {
        let clip = null;
        const nodeID = stringToName(id, path);
        if (source instanceof MediaElementAudioSourceNode) {
            clip = new AudioElementSource(stringToName("audio-clip-element", nodeID), this.audioCtx, source, randomize, this.createSpatializer(nodeID, spatialize, false), ...effectNames);
        }
        else {
            clip = new AudioBufferSpawningSource(stringToName("audio-clip-spawner", nodeID), this.audioCtx, source, randomize, this.createSpatializer(nodeID, spatialize, false), ...effectNames);
        }
        if (autoPlaying) {
            clip.play();
        }
