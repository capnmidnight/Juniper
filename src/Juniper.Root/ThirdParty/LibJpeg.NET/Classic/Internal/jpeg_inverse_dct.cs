...
namespace BitMiracle.LibJpeg.Classic.Internal
{
    ...
    internal class jpeg_inverse_dct
    {
        ...
        */
        ...
        private readonly inverse_method[] m_inverse_DCT_method = new inverse_method[JpegConstants.MAX_COMPONENTS];
        ...
        private readonly multiplier_table[] m_dctTables;
        ...
        */
                ...
                switch ((J_DCT_METHOD)method)
                {
                    ...
                    case J_DCT_METHOD.JDCT_IFAST:
                        ...
                        for (var i = 0; i < JpegConstants.DCTSIZE2; i++)
                        {
                            ifmtbl[i] = JpegUtils.DESCALE(
                                componentInfo.quant_table.quantval[i] * aanscales[i],
                    ...
                    case J_DCT_METHOD.JDCT_FLOAT:
                        ...
                        for (var row = 0; row < JpegConstants.DCTSIZE; row++)
                        {
                            ...
                            {
                                fmtbl[ii] = (float)(componentInfo.quant_table.quantval[ii] * aanscalefactor[row] * aanscalefactor[col] * 0.125);
                                ii++;
                            }
                        }
                        break;
        ...
        private void jpeg_idct_islow(int component_index, short[] coef_block, int output_row, int output_col)
        {
            ...
            for (var ctr = JpegConstants.DCTSIZE; ctr > 0; ctr--)
            {
                ...
                */
                ...
                {
                    /* AC terms all zero */
                    var dcval = SLOW_INTEGER_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 0],
                ...
                var z2 = SLOW_INTEGER_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 0],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 0]);
                var z3 = SLOW_INTEGER_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 4],
        ...
        private void jpeg_idct_ifast(int component_index, short[] coef_block, int output_row, int output_col)
        {
            ...
            for (var ctr = 0; ctr < JpegConstants.DCTSIZE; ctr++)
            {
                ...
                */
                ...
                {
                    /* AC terms all zero */
                    var dcval = limit[limitOffset + FAST_INTEGER_IRIGHT_SHIFT(z5, FAST_INTEGER_PASS1_BITS + 3) & RANGE_MASK];
        ...
        private void jpeg_idct_float(int component_index, short[] coef_block, int output_row, int output_col)
        {
            ...
            for (var ctr = JpegConstants.DCTSIZE; ctr > 0; ctr--)
            {
                ...
                */
                ...
                {
                    /* AC terms all zero */
                    var dcval = FLOAT_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 0],
                        quantTable[quantTableIndex + JpegConstants.DCTSIZE * 0]);
                ...
                var tmp0 = FLOAT_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 0],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 0]);
                var tmp1 = FLOAT_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 2],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 2]);
                var tmp2 = FLOAT_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 4],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 4]);
                var tmp3 = FLOAT_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 6],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 6]);
                ...
                var tmp4 = FLOAT_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 1],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 1]);
                var tmp5 = FLOAT_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 3],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 3]);
                var tmp6 = FLOAT_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 5],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 5]);
                var tmp7 = FLOAT_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 7],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 7]);
        ...
        private void jpeg_idct_16x16(int component_index, short[] coef_block, int output_row, int output_col)
        {
            ...
            {
                /* Even part */
                var tmp0 = SLOW_INTEGER_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 0],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 0]);
                tmp0 <<= SLOW_INTEGER_CONST_BITS;
                /* Add fudge factor here for final descale. */
                tmp0 += 1 << (SLOW_INTEGER_CONST_BITS - SLOW_INTEGER_PASS1_BITS - 1);

                var z1 = SLOW_INTEGER_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 4],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 4]);
                var tmp1 = z1 * SLOW_INTEGER_FIX(1.306562965);      /* c4[16] = c2[8] */
                ...
                z1 = SLOW_INTEGER_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 2],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 2]);
                var z2 = SLOW_INTEGER_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 6],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 6]);
                var z3 = z1 - z2;
                ...
                var tmp3 = z4 - z2 * SLOW_INTEGER_FIX(0.509795579); /* (c10-c14)[16] = (c5-c7)[8] */
        ...
        private void jpeg_idct_16x8(int component_index, short[] coef_block, int output_row, int output_col)
        {
            ...
            for (var ctr = JpegConstants.DCTSIZE; ctr > 0; ctr--)
            {
                ...
                */
                ...
                var z2 = SLOW_INTEGER_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 2],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 2]);
                var z3 = SLOW_INTEGER_DEQUANTIZE(coef_block[coefBlockIndex + JpegConstants.DCTSIZE * 6],
                    quantTable[quantTableIndex + JpegConstants.DCTSIZE * 6]);
