...
namespace Juniper.HTTP.Server.Controllers
{
    internal abstract class AbstractRegexRouteHandler : AbstractRouteHandler
    {
        ...
        protected AbstractRegexRouteHandler(string name, object source, MethodInfo method, RouteAttribute route)
        ...
        {
            return obj is AbstractRegexRouteHandler handler
                && base.Equals(obj)
        ...
        public override int CompareTo(AbstractRouteHandler other)
        {
            var compare = base.CompareTo(other);
            if (compare == 0 && other is AbstractRegexRouteHandler regexHandler)
            {
                // longer routes before shorter routes
                return -string.CompareOrdinal(regexSource, regexHandler.regexSource);
            }
            else
            {
                return compare;
