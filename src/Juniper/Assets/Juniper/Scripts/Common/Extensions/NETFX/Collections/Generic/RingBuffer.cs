...
namespace System.Collections.Generic
{
    ...
    public class RingBuffer<T> : IList<T>
    {
        ...
        /// </summary>
        /// <example>
        /// var buffer = new RingBuffer{int}(5); // buffer.Count == 0 buffer.Add(3); // buffer.Count
        /// == 1 buffer.Add(1); // buffer.Count == 2 buffer.Add(4); // buffer.Count == 3
        /// buffer.Add(1); // buffer.Count == 4 buffer.Add(5); // buffer.Count == 5 buffer.Add(9); //
        /// buffer.Count == 5 buffer.Add(2); // buffer.Count == 5
        /// </example>
        public int Count
        {
            get
        ...
        /// </exception>
        /// <example>
        /// var buffer = new RingBuffer{int}(3); buffer.Add(3); buffer.Add(1); buffer.Add(4); ///
        /// int[] arr = new int[3]; // arr -&gt; { 0, 0, 0 } buffer.CopyTo(arr, 0, 0, 3); // arr
        /// -&gt; { 3, 1, 4 } buffer.CopyTo(arr, 1, 0, 2); // arr -&gt; { 1, 4, 4 }
        /// buffer.CopyTo(arr, 0, 1, 2); // arr -&gt; { 1, 3, 1 } buffer.CopyTo(arr, 1, 0, 3); //
        /// throws ArgumentException buffer.CopyTo(arr, 0, 1, 3); // throws ArgumentException
        /// </example>
        ...
        /// <example>
        /// var buffer = new RingBuffer{int}(3); buffer.Add(3); buffer.Add(1); buffer.Add(4); ///
        /// int[] arr = new int[3]; // arr -&gt; { 0, 0, 0 } buffer.CopyTo(arr, 0, 3); // arr -&gt; {
        ...
        /// <example>
        /// var buffer = new RingBuffer{int}(3); buffer.Add(3); buffer.Add(1); buffer.Add(4); ///
        /// int[] arr = new int[4]; // arr -&gt; { 0, 0, 0, 0 } buffer.CopyTo(arr); // arr -&gt; { 3,
        /// 1, 4, 0 } int[] arr = new int[4]; // arr -&gt; { 0, 0, 0 } buffer.CopyTo(arr); // arr
        ...
        /// <example>
        /// var buffer = new RingBuffer{int}(3); buffer.Add(3); // { 3 } buffer.Add(1); // { 3, 1 }
        /// buffer.Add(4); // { 3, 1, 4 } buffer.Remove(1); // { 3, 4 } buffer.Add(5); // { 3, 4, 5 }
        /// buffer.Add(6); // { 4, 5, 6 } buffer.Remove(3); // { 4, 5, 6 } buffer.Remove(4); // { 5,
        /// 6 }
        /// </example>
        ...
        /// </remarks>
        /// <example>
        /// var buffer = new RingBuffer{int}(3); /// buffer.Add(3); buffer.IndexOf(3); // --&gt; 0
        /// buffer.IndexOf(1); // --&gt; -1 buffer.IndexOf(4); // --&gt; -1 buffer.IndexOf(1); //
        /// --&gt; -1 buffer.IndexOf(5); // --&gt; -1 /// buffer.Add(1); buffer.IndexOf(3); // --&gt;
        /// 0 buffer.IndexOf(1); // --&gt; 1 buffer.IndexOf(4); // --&gt; -1 buffer.IndexOf(1); //
        /// --&gt; 1 buffer.IndexOf(5); // --&gt; -1 /// buffer.Add(4); buffer.IndexOf(3); // --&gt;
        /// 0 buffer.IndexOf(1); // --&gt; 1 buffer.IndexOf(4); // --&gt; 2 buffer.IndexOf(1); //
        /// --&gt; 1 buffer.IndexOf(5); // --&gt; -1 /// buffer.Add(1); buffer.IndexOf(3); // --&gt;
        /// -1 buffer.IndexOf(1); // --&gt; 0 buffer.IndexOf(4); // --&gt; 1 buffer.IndexOf(1); //
        /// --&gt; 2 buffer.IndexOf(5); // --&gt; -1 /// buffer.Add(5); buffer.IndexOf(3); // --&gt;
        /// -1 buffer.IndexOf(1); // --&gt; 1 buffer.IndexOf(4); // --&gt; 0 buffer.IndexOf(1); //
        /// --&gt; 1 buffer.IndexOf(5); // --&gt; 2
        /// </example>
        ...
        /// </remarks>
        /// <example>
        /// var buffer = new RingBuffer{int}(4); buffer.Insert(0, 5); // { 5 } buffer.Insert(0, 7);
        /// // { 7, 5 } buffer.Insert(1, 3); // { 7, 3, 5 } buffer.Insert(1, 13); // { 7, 13, 3, 5 }
        /// buffer.Insert(2, 2); // { 13, 3, 2, 5 } buffer.Insert(0, 17); // { 17, 3, 2, 5 }
        /// </example>
        ...
        /// </exception>
        /// <example>
        /// var buffer = new RingBuffer{int}(4); buffer.Add(3); // { 3 } buffer.Add(1); // { 3, 1 }
        /// buffer.Add(4); // { 3, 1, 4 } buffer.Add(1); // { 3, 1, 4, 1 } buffer.Add(5); // { 3, 1,
        /// 4, 1, 5 } buffer.Add(9); // { 1, 4, 1, 5, 9 } buffer.Add(2); // { 4, 1, 5, 9, 2 }
        /// buffer.RemoveAt(2); // { 4, 1, 9, 2 } buffer.RemoveAt(0); // { 1, 9, 2 }
        /// buffer.RemoveAt(2); // { 1, 9 } buffer.RemoveAt(0); // { 9 } buffer.RemoveAt(0); // { }
