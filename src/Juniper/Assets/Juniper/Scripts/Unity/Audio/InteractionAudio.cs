...
namespace Juniper.Unity.Audio
{
    ...
    public class InteractionAudio : MonoBehaviour
    {
...
#if RESONANCEAUDIO
        private const string DEFAULT_AUDIO_MIXER = "ResonanceAudioMixer";
#elif OCULUS
        private const string DEFAULT_AUDIO_MIXER = "SpatializerMixer";
#else
        private const string DEFAULT_AUDIO_MIXER = "defaultAudioMixer";
#endif
            ...
            if (defaultMixerGroup == null)
            {
                defaultMixerGroup =
                    (from g in Resources.FindObjectsOfTypeAll<AudioMixerGroup>()
                     where g.audioMixer.name == DEFAULT_AUDIO_MIXER
        ...
        /// <returns></returns>
        protected AudioSource InternalSpatialize(AudioSource audioSource, bool loop, AudioMixerGroup group)
        {
