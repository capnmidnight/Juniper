...
public class XmlSpreadsheet2003
{
    ...
    private static readonly XName WORKSHEET = XName.Get("Worksheet", NAMESPACE);
    private static readonly XName TABLE = XName.Get("Table", NAMESPACE);
    private static readonly XName ROW = XName.Get("Row", NAMESPACE);
    private static readonly XName CELL = XName.Get("Cell", NAMESPACE);
    private static readonly XName DATA = XName.Get("Data", NAMESPACE);

    private static readonly XName NAME = XName.Get("Name", NAMESPACE);
    private static readonly XName INDEX = XName.Get("Index", NAMESPACE);
    ...
    public static async Task<XmlSpreadsheet2003> LoadAsync(FileInfo file, CancellationToken cancellationToken)
    {
        using var stream = new StreamReader(file.FullName, new FileStreamOptions
        {
            Mode = FileMode.Open,
        ...
        var root = await XElement.LoadAsync(stream, LoadOptions.None, cancellationToken);
        return new XmlSpreadsheet2003(file.Name, root);
    }
