...
public class NPMInstallCommand : ShellCommand
{
    ...
    private static bool CheckBeta(ref string version)
    {
        var isBeta = version.Contains("-beta");
        if (isBeta)
        {
    ...
    private static async Task<string?> NeedsInstall(DirectoryInfo fromDir, string name, string requiredVersionStr, CancellationToken cancellationToken)
    {
        ...
        var requiredIsBeta = CheckBeta(ref requiredVersionStr);
        ...
        var pkgVersion = package.version;
        ...
        var pkgIsBeta = CheckBeta(ref pkgVersion);

        if (!Version.TryParse(pkgVersion, out var actualVersion))
        {
            return $"Dependency {name} does not have a version value in package.json";
        }
        ...
        var isGood = requiredIsBeta == pkgIsBeta && op switch
        {
        ...
        if (!isGood)
        {
            return $"Dependency {name} versions don't match {package.name}: {pkgVersion} {op} {requiredVersionStr[op.Length..]}";
        }
