...
public class TempShellScriptCommand : ShellCommand, IDisposable
{
    ...
    private static TempFile WriteTempFile(string scriptContents, MediaType? contentType = null)
    {
    ...
    private static TempFile WriteTempFile(byte[] data, MediaType? contentType = null)
    {
        var tempFile = new TempFile(contentType);
        using var stream = tempFile.Create();
    ...
    /// to position the file path to the temp file in the argument list. If <see cref="TEMP_FILE_PATH_PLACEHOLDER"/>
    /// is not provided, the file path will be added to the end of the argument list.</param>
    public TempShellScriptCommand(DirectoryInfo workingDir, string scriptContents, string command, params string[] args)
        : base(workingDir, command, args)
    {
        tempFile = WriteTempFile(scriptContents);
        ReplaceFilePathPlaceholder();
    }
    ...
    /// to position the file path to the temp file in the argument list. If <see cref="TEMP_FILE_PATH_PLACEHOLDER"/>
    /// is not provided, the file path will be added to the end of the argument list.</param>
    public TempShellScriptCommand(string scriptContents, string command, params string[] args)
        : base(command, args)
    {
        tempFile = WriteTempFile(scriptContents);
        ReplaceFilePathPlaceholder();
    ...
    /// to position the file path to the temp file in the argument list. If <see cref="TEMP_FILE_PATH_PLACEHOLDER"/>
    /// is not provided, the file path will be added to the end of the argument list.</param>
    public TempShellScriptCommand(DirectoryInfo workingDir, byte[] fileContents, string command, params string[] args)
        : base(workingDir, command, args)
    {
        tempFile = WriteTempFile(fileContents);
        ReplaceFilePathPlaceholder();
    }
    ...
    /// to position the file path to the temp file in the argument list. If <see cref="TEMP_FILE_PATH_PLACEHOLDER"/>
    /// is not provided, the file path will be added to the end of the argument list.</param>
    public TempShellScriptCommand(byte[] fileContents, string command, params string[] args)
        : base(command, args)
    {
        tempFile = WriteTempFile(fileContents);
        ReplaceFilePathPlaceholder();
    ...
    {
        var anyFound = false;
        var filePath = tempFile.FilePath.QuotePath();
