...
namespace BitMiracle.LibJpeg.Classic.Internal
{
    ...
    /// </para>
    /// </summary>
    internal class JpegDCoefController
    {
        ...
        */
        ...
        private readonly JVirtArray<JBlock>[] m_coef_arrays;
        ...
        public JVirtArray<JBlock>[] GetCoefArrays()
        {
        ...
        /// </para>
        /// </summary>
        private ReadResult DecompressOnePass(ComponentBuffer[] output_buf)
        {
            ...
            for (var yoffset = m_MCU_vert_offset; yoffset < m_MCU_rows_per_iMCU_row; yoffset++)
            {
                for (var MCU_col_num = m_MCU_ctr; MCU_col_num <= last_MCU_col; MCU_col_num++)
                {
                    ...
                    for (var ci = 0; ci < m_cinfo.m_comps_in_scan; ci++)
                    {
                        ...
                        for (var yindex = 0; yindex < componentInfo.MCU_height; yindex++)
                        {
                            if (m_cinfo.inputIMcuRow < last_iMCU_row || yoffset + yindex < componentInfo.last_row_height)
                            {
        ...
        private ReadResult DecompressDataOrdinary(ComponentBuffer[] output_buf)
        {
            ...
            for (var ci = 0; ci < m_cinfo.numComponents; ci++)
            {
                ...
                var buffer = m_whole_image[ci].Access(m_cinfo.outputIMcuRow * componentInfo.V_samp_factor,
        ...
        private bool SmoothingOK()
        {
            ...
            for (var ci = 0; ci < m_cinfo.numComponents; ci++)
            {
                ...
                {
                    m_coef_bits_latch[m_coef_bits_savedOffset + coefi] = m_cinfo.coefBits[ci][coefi];
                    if (m_cinfo.coefBits[ci][coefi] != 0)
                    {
        ...
        private void StartiMCURow()
        {
            ...
            else
            {
                ...
                if (m_cinfo.inputIMcuRow < (m_cinfo.m_total_iMCU_rows - 1))
                {
