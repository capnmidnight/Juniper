...
/// </para>
/// </summary>
internal class JpegDMainController
{
    ...
    private readonly int[][][] m_funnyIndices = [new int[JpegConstants.MAX_COMPONENTS][], new int[JpegConstants.MAX_COMPONENTS][]];
    ...
    public void ProcessData(byte[][] output_buf, ref int out_row_ctr, int out_rows_avail)
    {
    ...
    /// </summary>
    private void ProcessDataCrankPost(byte[][] output_buf, ref int out_row_ctr, int out_rows_avail)
    {
        var dummy = 0;
        m_cinfo.m_post.PostProcessData(null, ref dummy, 0, output_buf, ref out_row_ctr, out_rows_avail);
    }
    ...
    /// </summary>
    private void AllocFunnyPointers()
    {
        var M = m_cinfo.min_DCT_v_scaled_size;
        for (var ci = 0; ci < m_cinfo.numComponents; ci++)
        {
            /* height of a row group of component */
            var rgroup = (m_cinfo.CompInfo[ci].V_samp_factor * m_cinfo.CompInfo[ci].DCT_v_scaled_size) / m_cinfo.min_DCT_v_scaled_size;
    ...
    /// </summary>
    private void MakeFunnyPointers()
    {
        var M = m_cinfo.min_DCT_v_scaled_size;
        for (var ci = 0; ci < m_cinfo.numComponents; ci++)
        {
    ...
    /// </summary>
    private void SetWraparoundPointers()
    {
        var M = m_cinfo.min_DCT_v_scaled_size;
        for (var ci = 0; ci < m_cinfo.numComponents; ci++)
        {
    ...
    /// </summary>
    private void SetBottomPointers()
    {
        for (var ci = 0; ci < m_cinfo.numComponents; ci++)
        {
            /* Count sample rows in one iMCU row and in one row group */
            var iMCUheight = m_cinfo.CompInfo[ci].V_samp_factor * m_cinfo.CompInfo[ci].DCT_v_scaled_size;
            var rgroup = iMCUheight / m_cinfo.min_DCT_v_scaled_size;

            /* Count nondummy sample rows remaining for this component */
            var rows_left = m_cinfo.CompInfo[ci].downsampled_height % iMCUheight;
            if (rows_left == 0)
            {
