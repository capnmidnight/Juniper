...
/// </para>
/// </summary>
internal class My1PassCQuantizer : IJpegColorQuantizer
{
    ...
    */
    private static readonly byte[][] base_dither_matrix =
    [
    ...
    */
    ...
    private readonly int[] m_Ncolors = new int[MAX_Q_COMPS];  /* # of values alloced to each component */

    /* Variables for ordered dithering */
    private int m_row_index;    /* cur row's vertical index in dither matrix */
    private readonly int[][][] m_odither = new int[MAX_Q_COMPS][][]; /* one dither array per component */

    /* Variables for Floyd-Steinberg dithering */
    private readonly short[][] m_fserrors = new short[MAX_Q_COMPS][]; /* accumulated errors */
    ...
    {
        var nc = m_cinfo.outColorComponents;

        for (var row = 0; row < num_rows; row++)
        {
            var inIndex = 0;
            var inRow = in_row + row;
    ...
    /// </summary>
    private void QuantizeOrdDither(byte[][] input_buf, int in_row, byte[][] output_buf, int out_row, int num_rows)
    {
        var nc = m_cinfo.outColorComponents;
        var width = m_cinfo.outputWidth;
    ...
    /// </summary>
    private void Quantize3OrdDither(byte[][] input_buf, int in_row, byte[][] output_buf, int out_row, int num_rows)
    {
        var width = m_cinfo.outputWidth;
    ...
    /// </summary>
    private void QuantizeFsDither(byte[][] input_buf, int in_row, byte[][] output_buf, int out_row, int num_rows)
    {
        var nc = m_cinfo.outColorComponents;
        var width = m_cinfo.outputWidth;
    ...
    /// </summary>
    private void CreateODitherTable()
    {
        for (var i = 0; i < m_cinfo.outColorComponents; i++)
        {
            var nci = m_Ncolors[i]; /* # of distinct values for this color */
            ...
            {
                /* need a new table? */
                m_odither[i] = MakeODitherArray(nci);
            }
            else
            {
    ...
    /// </summary>
    private void AllocFsWorkspace()
    {
        for (var i = 0; i < m_cinfo.outColorComponents; i++)
        {
            m_fserrors[i] = new short[m_cinfo.outputWidth + 2];
        }
    }
    ...
    /// </summary>
    private static int LargestInputValue(int j, int maxj)
    {
    ...
    /// </summary>
    private static int OutputValue(int j, int maxj)
    {
    ...
    /// </summary>
    private int SelectNColors(int[] Ncolors)
    {
        var nc = m_cinfo.outColorComponents; /* number of color components */
        var max_colors = m_cinfo.desiredNumberOfColors;
        ...
        if (iroot < 2)
        {
            m_cinfo.ErrExit(JMessageCode.JERR_QUANT_FEW_COLORS, (int)temp);
        }
        ...
        do
        {
            ...
            for (var i = 0; i < nc; i++)
            {
                var j = (m_cinfo.outColorSpace == JColorSpace.JCS_RGB ? RGB_order[i] : i);
    ...
    /// </summary>
    private static int[][] MakeODitherArray(int ncolors)
    {
        var odither = new int[ODITHER_SIZE][];
        for (var i = 0; i < ODITHER_SIZE; i++)
        {
         ...
         */
        var den = 2 * ODITHER_CELLS * (ncolors - 1);
        for (var j = 0; j < ODITHER_SIZE; j++)
        {
            for (var k = 0; k < ODITHER_SIZE; k++)
            {
                var num = (ODITHER_CELLS - 1 - (2 * base_dither_matrix[j][k])) * JpegConstants.MAXJSAMPLE;
