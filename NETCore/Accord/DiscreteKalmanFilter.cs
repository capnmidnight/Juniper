...
public class DiscreteKalmanFilter<TState, TMeasurement>
{
    ...
    /// </para>
    /// </summary>
    public void Predict(double[]? controlVector = null)
    {
        CheckPrerequisites();
        //x'(k) = A * x(k-1)
        ...
        //P'(k) = A * P(k-1) * At + Q
        EstimateCovariance = TransitionMatrix
            .Multiply(EstimateCovariance)
        ...
        //S(k) = H * P'(k) * Ht + R
        ResidualCovariance = MeasurementMatrix
            .Multiply(EstimateCovariance)
            .Multiply(measurementMatrixTransponsed)
            .Add(MeasurementNoise);
        ResidualCovarianceInv = ResidualCovariance.Inverse();

        //K(k) = P'(k) * Ht * S(k)^(-1)
        KalmanGain = EstimateCovariance
            .Multiply(measurementMatrixTransponsed)
            .Multiply(ResidualCovarianceInv);
        /******* calculate Kalman gain **********/
    }
