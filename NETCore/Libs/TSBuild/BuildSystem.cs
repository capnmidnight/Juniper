    ...
    where BuildConfigT : IBuildConfig, new()
{
    ...
    static void WriteError(string format, params object[] values) =>
        Console.Error.WriteLine(format.Interpolate(values).RemoveClearScreen().Colorize("error", 31));

    static void WriteInfo(string format, params object[] values) =>
        Console.WriteLine(format.Interpolate(values).RemoveClearScreen().Colorize("info", 32));

    static void WriteWarning(string format, params object[] values) =>
        Console.WriteLine(format.Interpolate(values).RemoveClearScreen().Colorize("warn", 33));
