...
public class UniversalControllerActions : MonoBehaviour {
    ...
    void Update()
    {
        ...
        {
            Log.d(LOG_TAG, "WVR_InputId_Alias1_Touchpad press down");
            isTouchPressed = true;
            if (Touch_Press != null)
            {
        ...
        {
            Log.d(LOG_TAG, "WVR_InputId_Alias1_Touchpad press up");
            isTouchPressed = false;
            if (Touch_Press != null)
            {
                Touch_Press.SetActive(false);
        ...
        if (WaveVR_Controller.Input(this.device).GetTouch(WVR_InputId.WVR_InputId_Alias1_Touchpad))
        {
            if (!isTouchPressed)
            {
                ...
                if (TouchPad != null && Touch_Dot != null)
                {
                    ...
                    Log.gpl.d(LOG_TAG, "Touch_EffectMesh. x scale: " + Touch_Dot.transform.localScale.x);
                    Log.gpl.d(LOG_TAG, "Touch_EffectMesh. y scale: " + Touch_Dot.transform.localScale.y);
