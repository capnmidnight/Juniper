...
public static partial class Program
{
    ...
    private static void PromotePrimaryExtension(Dictionary<string, Group> groups, string groupName, string entryName, string ext)
    {
        if (groups.TryGetValue(groupName, out Group? group) 
            && group.Entries.TryGetValue(entryName, out Entry? entry))
        {
    ...
    public static async Task Main2(string[] args)
    {
        if (args?.Length != 1)
        {
            Console.WriteLine("Usage: <exe> C:\\path\\to\\output\\dir");
            return;
        }
        ...
        if (!outDir.Exists)
        {
            Console.WriteLine("Directory not found: " + outDir.FullName);
            return;
        }
